...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            df = pd.read_csv('credit_risk_prediction_1500.csv')
            logging.info("Read the credit risk dataset as dataframe")

            # Convert target Yes/No -> 1/0 for regression
            df['Grant_Loan'] = df['Grant_Loan'].map({'Yes': 1, 'No': 0})
            logging.info("Converted target column Grant_Loan: Yes->1, No->0")
